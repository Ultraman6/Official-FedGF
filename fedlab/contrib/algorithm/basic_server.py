...
class SyncServerHandler(ServerHandler):
    ...
    def load(self, payload: List[torch.Tensor], upload_res=False):
        ...
        if len(self.client_buffer_cache) == self.num_clients_per_round:
            ...
            # reset cache
            # if upload_res:
            #     self.calc_grad_cos(self.client_buffer_cache)
    ...
    def calc_grad_cos(self, buffer):
        gradient_list = [torch.sub(ele[0], self.model_parameters) for ele in buffer]
        tot_norm = 0
        for grad in gradient_list:
            tot_norm += grad.norm(2).item()
        tot_norm /= len(gradient_list)

        avg_grad = torch.mean(torch.stack(gradient_list, dim=0), dim=0)
        # self.cos_sim = torch.mean([cos(avg_grad, gradient) for gradient in gradient_list])
        cos = CosineSimilarity(dim=0, eps=1e-6)
        res = []
        for gradient in gradient_list:
            res.append(cos(avg_grad, gradient).item())

        self.cos_sim = torch.mean(torch.tensor(res, dtype=float)).item()
        self.norm_grad = round(tot_norm, 2)
    ...
    def calc_cosine_similarity(self, buffer):
        # weights = [float(1/len(buffer))] * len(buffer)
        #
        # weights = torch.tensor(weights)
        # weights = weights / torch.sum(weights)

        # gradient is theta_prev - theta_current
        gradient_list = [
            torch.sub(self.model_parameters, ele[0]) for idx, ele in enumerate(buffer)
        ]
